...
router.get(
  ...
  RegistrarActivityController.getDataByEncodedSemester
);
...
router.get("/getLastGradeSheetSubmittedLog", async (req, res) => {
  ...
  const conn = await startConnection(req);
  try {
    const rows = await getSingleSetOfData(
      conn,
      "tbl_class_update_logs",
      "class_code",
      class_code
    );
...
router.post("/updateGraduateStudiesGrade", async (req, res) => {
  ...
  const modifiedEventKey = await insertModifiedEventLog(
    conn,
    "modified_eventlog",
    "student_grades",
    userName,
  ...
  const countAffectedRows = async (gradeData) => {
    try {
      const rows = await indexUpdateGraduateStudiesGrade(
        conn,
        gradeData,
        modifiedEventKey
      );
...
router.post("/updateGrade", async (req, res) => {
  ...
  const countAffectedRows = async (grade) => {
    const userName = await eventkeyUserEmailRef(conn, email_used); // Get User Full Name using Email as Preference
    try {
      const modifiedEventKey = await insertModifiedEventLog(
        conn,
        "modified_eventlog",
  ...
  async (req, res) => {
    ...
    try {
      ...
      const processRow = async (rowData) => {
        try {
          const rows = await checkIfHasRemarkInGradeSheet(
            conn,
            rowData,
            subjectCode,
            modifiedEventKey
...
router.post(
  ...
  async (req, res) => {
    ...
    if (sheetName === decodeClassCode) {
      ...
      const processRow = async (rowData) => {
        try {
          const rows = await checkIfHasRemarkInGSGradeSheet(
            conn,
            rowData,
            subjectCode,
            modifiedEventKey
...
router.post("/submitGradeSheet", async (req, res) => {
  ...
  const conn = await startConnection(req);
  try {
    response = await indexUpdateClassCodeStatus(
      conn,
      email_used,
      class_code,
      term_type
