...
LEFT JOIN graduate_studies gs
    ON gs.subject_code = c.subject_code
LEFT JOIN section s 
    ON c.section_id = s.section_id
LEFT JOIN student_load sl 
    ON c.class_code = sl.class_code
LEFT JOIN updates u 
    ON u.class_code = c.class_code
LEFT JOIN upload_grade_extensions uge 
    ON uge.class_code = c.class_code
LEFT JOIN tbl_class_update_logs ul 
    ON ul.class_code = c.class_code
LEFT JOIN class_code_status ccs 
    ON ccs.class_code = c.class_code
LEFT JOIN registrar_activity_online rao 
    ON c.school_year = rao.schoolyear AND c.semester = rao.semester
WHERE c.faculty_id = ?
  AND c.school_year = ?
  AND c.semester = ?
${query}
