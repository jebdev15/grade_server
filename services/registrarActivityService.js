...
const RegistrarActivityService = {
  getData: async (conn) => {
    const [rows] =
      await conn.query(`SELECT *, \`to\` >= CURDATE() AS withinDuration
        FROM registrar_activity_online
        ORDER BY id`);
    return rows;
  },
  getDataBySemester: async (conn, req) => {
    const [rows] = await conn.query(
      `SELECT * FROM registrar_activity_online WHERE semester = ?`,
      [req.query.semester]
    );
    return rows;
  },
  ...
  updateDataById: async (conn, req) => {
    ...
    const [rows] = await conn.query(
      "UPDATE registrar_activity_online SET activity = ?, schoolyear = ?, semester = ?, status = ?, `from` = ?, `to` = ?, currentSem = ?, term_type = ? WHERE id = ?",
      [
        activity,
        schoolyear,
        semester,
    ...
    await conn.query(
      `UPDATE class_code_status 
        SET \`${columnName}\` = ? 
        WHERE class_code IN (SELECT class_code FROM class WHERE school_year = ? AND semester = ?)`,
      [0, schoolyear, semester]
    );
    return rows;
  },
};
